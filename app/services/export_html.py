...
def export_html():
    ...
    if not HAS_SFTP:
        raise ValidationError(f"SFTP är inte konfigurerat. HTML har endast sparats lokalt.\nÄndra konfiguration i {CONFIG_FILE}.")
    if not HAS_HTML:
        raise ValidationError(f"Fjärrmapp (HTML - REMOTE_DIR) är inte konfigurerat. HTML har endast sparats lokalt.\nÄndra konfiguration i {CONFIG_FILE}.")
    try:
